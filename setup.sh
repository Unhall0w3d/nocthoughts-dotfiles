...
if [[ $install_packages == "Y" || $install_packages == "y" ]]; then
    ...
    for pkg in exa mailspring krita eww-wayland gtk gtk2 gtk3 gtk4 ruby htop materia-transparent-gtk-theme-git socat mpvpaper inotify-tools webkit2gtk webkit2gtk-4.1 webkit2gtk-5.0 vlc bat brave-bin qt5-styleplugins qt5ct archlinux-tweak-tool-git mpv wget curl polkit polkit-gnome pavucontrol-qt pamixer pipewire pipewire-audio pipewire-jack pipewire-pulse lib32-pipewire lib32-libpipewire jack wireplumber brightnessctl bluez bluez-utils blueman blueberry network-manager-applet gvfs stacer thunar thunar-archive-plugin thunar-shares-plugin thunar-vcs-plugin thunar-volman file-roller neofetch neovim ranger fnm noise-suppression-for-voice viewnior cava ripgrep ffmpegthumbnailer btop dunst wl-clipboard wf-recorder hyprpicker-git hyprpaper-git tumbler imagemagick ncspot spotify-tui pix; do
        if ! pacman -Qs "$pkg" > /dev/null ; then
            echo -e "$CNT - Installing $pkg... " 
            paru -S --needed --noconfirm "$pkg"
        else
    ...
    # Install Eww-Wayland
    echo -e "$CNT - Installing Eww-wayland... "
